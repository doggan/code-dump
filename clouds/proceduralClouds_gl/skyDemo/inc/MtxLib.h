...
/* Notes -
    The library contains 2 main namespaces: _mtxCM and _quat - For column-major and
    quaternion, respectively. Any matrix to quaternion or or quaternion to matrix operation
    (in short, anything that involves a quaternion) is handled by the quaternion namespace.
    The matrix classes are unknowing of anything called a quaternion.

    The _mtxCM namespace contains five main classes: vector2, vector3, vector4, matrix33
    and matrix44. Vectors have public members called x, y, z, and w, and matrices are
    composed of arrays of vectors. Most of the standard matrix and vector operations have
    been overloaded; however, the cross product and dot product operations are provided as
    utility functions for a more coherent notation. If you need to load a matrix onto the
    OpenGL matrix stack, it is safe to cast a *matrix to a *float (and, obviously,
    *vector to *float).

    TODO: _quat notes.

    As library was originally designed for OpenGL usage, all matrices/vectors are stored in
    a column-major fashion, as well as all utility functions being designed to work on these
    data-structures. The library can be easily used in a row-major system, however, through
    a few well-placed TransposeMatrix() calls. ;)
*/
